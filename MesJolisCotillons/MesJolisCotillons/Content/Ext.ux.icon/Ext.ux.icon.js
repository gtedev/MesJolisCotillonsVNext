...
Mjc.Icon = function (icon, iconsBase) {
    var base = iconsBase || 'general'
    return Common.ui.IconMgr.getIconBase(base, icon + '.png')
};
Mjc.Icon.setIconsBase = function (setName, path) {
    Common.ui.IconMgr.setIconsBase(setName, path);
};
/**
...
Ext.define('Common.ui.IconMgr', {
     ...
     * @property {String}
     */
    _ruleBodyTpl: ' .{0} { background-image: url( {1} ) !important; background-repeat: no-repeat;  }',
    ...
    _generateIconCSSRule: function (baseName, icon) {
        ...
        var cls = this.PREFIX + baseName + '_' + Ext.id();

        // Get icon image path
        var iconImgPath = this._iconsBases[baseName].path + '/' + icon;

        // Generate style text
        var styleBody = Ext.String.format(this._ruleBodyTpl, cls, iconImgPath);
